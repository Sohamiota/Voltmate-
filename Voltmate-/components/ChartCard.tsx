    ...
    <div className="bg-card border border-border rounded-xl p-6">
      ...
      <div className="flex gap-6 mt-6 pt-6 border-t border-border">
        <div>
          ...
          <p className="text-lg font-bold text-foreground">{hasData ? `$${data.reduce((s:any, d:any)=>s + (d.sales||0), 0).toLocaleString()}` : '-'}</p>
        </div>
        <div>
          <p className="text-xs text-muted-foreground mb-1">Target</p>
          <p className="text-lg font-bold text-secondary">{hasData ? `$${data.reduce((s:any, d:any)=>s + (d.target||0), 0).toLocaleString()}` : '-'}</p>
        </div>
      </div>
    </div>
  )
