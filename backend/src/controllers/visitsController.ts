...
// Auto-add phone columns to visits table if not present
...
export async function createVisit(req: Request, res: Response) {
  try {
    ...
    const r = await query(
      `INSERT INTO visits
        (lead_id, lead_cust_code, salesperson_id, vehicle, status, visit_date, next_action, next_action_date, note, phone_no, phone_no_2, created_by, created_at, updated_at)
      ...
      [lead_id, lead.cust_code, salesperson_id || null, vehicle || null, status || null,
       ...
       phone_no || null, phone_no_2 || null, userId || null]
    );
...
export async function listVisits(req: Request, res: Response) {
  ...
  try {
    await ensureVisitsCols();
    const r = await query(
      `SELECT v.*, l.cust_name, l.phone_no AS lead_phone_no, u.name as salesperson_name
       FROM visits v
...
export async function exportVisitsCSV(req: Request, res: Response) {
  try {
    await ensureVisitsCols();
    const r = await query(`SELECT v.id, v.lead_cust_code, l.cust_name, v.phone_no, v.phone_no_2,
      v.salesperson_id, u.name as salesperson_name,
    ...
    const header = ['id','lead_cust_code','cust_name','phone_no','phone_no_2','salesperson_id','salesperson_name','vehicle','status','visit_date','next_action','next_action_date','note','created_at'];
